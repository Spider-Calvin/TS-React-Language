  ...
  return (
    <div className="resultblock">
      <div className="flex gap-x-4 items-center">
        <a className="smallbtn bg-indigo-500 hover:bg-indigo-700" onClick={()=>Navigate(`/quiz?language=${params}`)}>Back</a>
        <p className="text-base ld:text-lg">You have scored {correctAns} out of {words.length}</p>
      </div>
      <div className="resulttextbox">
        ...
        <div className="flex gap-x-4">
          <div className="flex flex-col">
            ...
            {result.map((option,i)=><a key={i} className={`${ option === words[i].meaning ? 'text-green-800' : 'text-red-800' }`}>{i+1} {option}</a> )}
          </div>
          <div className="flex flex-col">
      ...
      <div className="flex gap-x-4 items-center lg:w-6/12">
        <a className={`smallbtn leading-7 self-end ${percentage >50 ? 'bg-green-600' :'bg-indigo-700'}`}>{percentage >50?'Pass':'Fail'}</a><a className={`smallbtn leading-7 self-end  ml-auto bg-indigo-500 hover:bg-indigo-700`} onClick={resetHandler}>Learn Again</a>
      </div>
    </div>
  )
}
